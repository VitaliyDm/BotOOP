...
public class Dialog{
    ...
    public void mainDialog(boolean gameStarted) throws IOException, InterruptedException {
        Boolean questionShowed = false;
        ioManager.write(showHelp());
        while (true){
            String userAnswer = ioManager.read();
    ...
    protected String showHelp(){
        String botInfo = "Вы можете сыграть с ботом в игру: \"Что? Где? Когда?\"";
        String botCommandsInfo = String.format("Чтоб начать игру введите \"%s\", для завершения игры введите: \"%s\"," +
